...
defineOptions({
  name: 'TicketLoginBatchList'
});

const appStore = useAppStore();

function createSearchParams(): Api.Ticket.LoginBatchSearchParams {
  return {
    pageNum: 1,
    pageSize: 10,
    platformId: null,
...
const searchParams = ref<Api.Ticket.LoginBatchSearchParams>(createSearchParams());
const platformOptions = ref<{ label: string; value: CommonType.IdType }[]>([]);
const detailVisible = ref(false);
const detailLoading = ref(false);
const currentBatch = ref<Api.Ticket.LoginBatch | null>(null);
const currentDetails = ref<Api.Ticket.LoginBatchDetail[]>([]);
...
const { columns, columnChecks, data, getData, getDataByPage, loading, mobilePagination, scrollX } =
  useNaivePaginatedTable({
    api: () => fetchGetTicketLoginBatchList(searchParams.value),
...
const detailColumns: NaiveUI.TableColumn<Api.Ticket.LoginBatchDetail>[] = [
  ...
  {
    key: 'sessionExpireTime',
    title: '会话到期',
    align: 'center',
    minWidth: 160,
    render: row => renderTicketEllipsis(row.sessionExpireTime)
  },
  {
    key: 'resultMessage',
    title: '返回信息',
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <NCard title="登录批次筛选" :bordered="false" size="small" class="card-wrapper">
    ...
    <NCard title="登录批次列表" :bordered="false" size="small" class="card-wrapper sm:flex-1-hidden">
    ...
    <NModal v-model:show="detailVisible" preset="card" title="登录批次明细" class="w-1000px">
        ...
        <div class="flex-col-stretch gap-16px">
          <NCard size="small" :bordered="false">
            <NGrid :cols="24" :x-gap="16">
              ...
              <NFormItemGi :span="8" label="总数">{{ currentBatch?.totalCount ?? 0 }}</NFormItemGi>
              <NFormItemGi :span="8" label="成功">{{ currentBatch?.successCount ?? 0 }}</NFormItemGi>
              <NFormItemGi :span="8" label="失败">{{ currentBatch?.failedCount ?? 0 }}</NFormItemGi>
            </NGrid>
          </NCard>
